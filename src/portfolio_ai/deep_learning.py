"""Train a small CNN on a labelled image directory with split-leakage checks."""
...
def train(root: Path, output: Path, epochs: int = 15, batch_size: int = 32,
    ...
    model = nn.Sequential(nn.Conv2d(3, 32, 3, padding=1), nn.ReLU(), nn.MaxPool2d(2),
                          nn.Conv2d(32, 64, 3, padding=1), nn.ReLU(), nn.MaxPool2d(2),
                          nn.Conv2d(64, 128, 3, padding=1), nn.ReLU(),
                          nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(128, len(classes))).to(device)
